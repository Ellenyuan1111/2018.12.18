...
var ranger = document.getElementById("gl2");
var textEle = document.getElementById("gl1");
var increase = document.getElementById('increase');
var reduce = document.getElementById('reduce');

increase.onclick = function(){   //点击增加按钮
    if(textEle.value >= ranger.max){
        alert('已达到最大值');
        return false;
    }else{
        textEle.value++;
        ranger.value = textEle.value;
        transformData(textEle.value);
    }
    
}
reduce.onclick = function(){    //点击减少按钮
    if(textEle.value <= ranger.min){
        alert('已达到最小值');
    }else{
        textEle.value--;
        ranger.value = textEle.value;
...
ranger.oninput = function (event) {   //滚动时触发的事件
    textEle.value = this.value;
    transformData(this.value);
}
// ranger.addEventListener("input", function (value) {});


function transformData(val){   //得到文本框里的数值
    dividingRule.position.z = -(val-2500)/50+0.3;
    measureBtton.traverse(function(child){
        if(child.isMesh){
            child.rotation.z = THREE.Math.degToRad(val*2);
        }
    });
}
